<div class="min-w-0">
    <div class="mb-2 flex items-center gap-1">
        <label for="recording-date" class="block text-sm font-medium text-slate-900 dark:text-white">
            {{__("Recording date")}}
        </label>
    ...
    <div class="relative w-full">
        ...
        <input id="recording-date" datepicker datepicker-autohide datepicker-format="yyyy-mm-dd"
               ...
               aria-invalid="@error('recording_date') true @else false @enderror"
               @error('recording_date') aria-describedby="recording-date-error" @enderror
               placeholder="{{__("Select date")}}" required>
    </div>
    @error('recording_date')
    <p id="recording-date-error" class="mt-2 text-sm leading-6 text-red-600">{{__("This is a required input")}}</p>
    @enderror
</div>
